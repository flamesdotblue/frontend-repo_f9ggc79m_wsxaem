...
export default function CourseSelect({ onPickPath }) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-black p-6">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(40%_40%_at_10%_10%,rgba(76,154,255,0.12),transparent_60%),radial-gradient(40%_40%_at_90%_90%,rgba(139,92,246,0.12),transparent_60%)]" />

      {/* Top bar */}
      <div className="relative mx-auto mb-8 flex max-w-6xl items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-4 py-3 shadow-lg backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <div className="text-sm text-slate-200">Hello, Learner</div>
        </div>
        <div className="hidden items-center gap-4 md:flex">
          <Home className="h-5 w-5 text-slate-400 hover:text-[#4C9AFF]" />
          <BarChart3 className="h-5 w-5 text-slate-400 hover:text-[#4C9AFF]" />
          <Settings className="h-5 w-5 text-slate-400 hover:text-[#4C9AFF]" />
        </div>
        <div className="flex w-40 items-center gap-2">
          <div className="h-2 flex-1 rounded-full bg-white/10">
            <div className="h-2 w-1/3 rounded-full bg-gradient-to-r from-[#4C9AFF] to-[#8B5CF6]" />
          </div>
          <span className="text-xs font-medium text-slate-300">36%</span>
        </div>
      </div>
            ...
            >
              {courses.map((c) => (
                <motion.button
                  ...
                  whileHover={{ y: -6 }}
                  whileTap={{ scale: 0.98 }}
                  className="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-5 text-left shadow-xl backdrop-blur-xl transition"
                >
                  ...
                  <div className="mb-2 text-lg font-semibold text-white">{c.title}</div>
          ...
          {selectedCourse && (
            ...
            >
              <div className="mb-6 flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-semibold text-white">Choose your learning path</h2>
                  <p className="text-sm text-slate-300">Course selected: {selectedCourse.title}</p>
                </div>
                <button
                  onClick={() => setSelectedCourse(null)}
                  className="rounded-full bg-white/10 px-4 py-2 text-sm text-slate-200 ring-1 ring-white/10 backdrop-blur-md transition hover:bg-white/15"
                >
                  Change course
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                {paths.map((p) => (
                  <motion.button
                    ...
                    whileHover={{ y: -6 }}
                    whileTap={{ scale: 0.98 }}
                    className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-6 text-left shadow-xl backdrop-blur-xl transition"
                  >
                    <div className="absolute -right-16 -top-16 h-52 w-52 rounded-full bg-gradient-to-br from-[#4C9AFF]/30 to-[#8B5CF6]/30 opacity-40 blur-2xl" />
                    <div className="mb-2 text-lg font-semibold text-white">{p.level}</div>
                    <div className="mb-4 text-sm text-slate-300">{p.desc}</div>
